...
fn markov() -> &'static MarkovChain {
  static MARKOV: sync::OnceLock<MarkovChain> = sync::OnceLock::new();
  MARKOV.get_or_init(|| {
...
async fn main() -> Result<(), lambda_runtime::Error> {
  // initialize the markov during cold-start, not first execution
  // I don't think this ends up making any practical difference
  // it's just a little more aesthetic
  let _ = markov();
